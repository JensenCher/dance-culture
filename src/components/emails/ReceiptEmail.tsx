...
export const ReceiptEmail = ({ date, email, orderId, products }: ReceiptEmailProps) => {
  const total = products.reduce((total, prod) => total + prod.price, 0) + 1;
            ...
            return (
              <Section key={product.id}>
                <Column style={{ width: "64px" }}>
                  {typeof image !== "string" && image.url ? <Img src={image.url} width="64" height="64" alt="Product Image" style={productIcon} /> : null}
                </Column>
                <Column style={{ paddingLeft: "22px" }}>
                  <Text style={productTitle}>{product.name}</Text>
                  {product.description ? (
                    <Text style={productDescription}>{product.description.length > 50 ? product.description?.slice(0, 50) + "..." : product.description}</Text>
                  ) : null}
